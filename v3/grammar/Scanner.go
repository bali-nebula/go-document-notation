...
const (
	...
	day_            = "(?:([0-2][1-9])|(3[0-1]))"
	days_           = "(?:(?:" + timespan_ + ")D)"
	delimiter_      = "(?:xor|with|while|to|throw|send|select|save|san|return|retrieve|reject|publish|on|notarize|not|matching|matches|loop|level|let|is|ior|in|if|from|each|do|discard|continue|checkout|break|at|and|accept|\\}|\\||\\{|\\^|\\]|\\[|\\?=|\\.\\.|\\+=|\\+|\\*=|\\*|\\)|\\(|@|>|=|<~|<-|<|:=|:|/=|/|-=|-|&|%)"
